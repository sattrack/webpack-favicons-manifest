...
(() => {
    ...
    function favicons(source, parameters, next) {
        ...
        function createFavicon(sourceset, properties, name, platformOptions, callback) {
            if (path.extname(name) === '.ico') {
                async.map(properties.sizes, (sizeProperties, cb) => {
                    ...
                    toIco(files).then(buffer => callback(null, { name, contents: buffer })).catch(callback);
                });
            } else {
                ...
                async.waterfall([cb => µ.Images.nearest(sourceset, properties, offset, cb), (nearest, cb) => µ.Images.read(nearest.file, cb), (buffer, cb) => µ.Images.resize(buffer, properties, offset, cb), (resizedBuffer, cb) => µ.Images.create(properties, background, (error, canvas) => cb(error, resizedBuffer, canvas)), (resizedBuffer, canvas, cb) => µ.Images.composite(canvas, resizedBuffer, properties, offset, maximum, cb), (composite, cb) => {
        ...
        function createHTML(platform, callback) {
            ...
            async.forEachOf(options.html[platform], (tag, selector, cb) => µ.HTML.parse(tag, (error, metadata) => cb(html.push(metadata) && error)), error => callback(error, html));
        }

        function createFiles(platform, platformOptions, callback) {
            const files = [];

            async.forEachOf(config.files[platform], (properties, name, cb) => µ.Files.create(properties, name, platformOptions, (error, file) => cb(files.push(file) && error)), error => callback(error, files));
        }

        function createFavicons(sourceset, platform, platformOptions, callback) {
            const images = [];

            async.forEachOf(config.icons[platform], (properties, name, cb) => createFavicon(sourceset, properties, name, platformOptions, (error, image) => cb(images.push(image) && error)), error => callback(error, images));
        }

        function createPlatform(sourceset, platform, platformOptions, callback) {
            async.parallel([cb => createFavicons(sourceset, platform, platformOptions, cb), cb => createFiles(platform, platformOptions, cb), cb => createHTML(platform, cb)], (error, results) => callback(error, results[0], results[1], results[2]));
        }
        ...
        function unpack(pack, callback) {
            ...
            async.each(pack.files, (url, cb) => µ.RFG.fetch(url, (error, box) => cb(response.images.push(box.image) && response.files.push(box.file) && error)), error => callback(error, response));
        }

        function createOnline(sourceset, callback) {
            async.waterfall([cb => µ.RFG.configure(sourceset, config.rfg, cb), (request, cb) => µ.RFG.request(request, cb), (pack, cb) => unpack(pack, cb)], (error, results) => {
        ...
        async.waterfall([callback => µ.General.source(source, callback), (sourceset, callback) => create(sourceset, callback), (response, callback) => {
            if (options.pipeHTML) {
        ...
        }], (error, response) => error ? next(error) : next(null, {
    ...
    function stream(params, handleHtml) {
        ...
        function processDocuments(documents, html, callback) {
            async.each(documents, (document, cb) => µ.HTML.update(document, html, config.html, cb), error => callback(error));
        }
        ...
        return through2.obj(function (file, encoding, callback) {
            ...
            async.waterfall([cb => favicons(file.contents, params, cb), (response, cb) => async.each(response.images.concat(response.files), (image, c) => {
                that.push(µ.General.vinyl(image, file));
                c();
            }, error => cb(error, response)), (response, cb) => {
                if (handleHtml) {
