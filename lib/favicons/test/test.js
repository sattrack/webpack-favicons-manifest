...
(() => {
    ...
    favicons('./logo.png', {
        ...
        if (response.images) {
            ...
            response.images.forEach(image => fs.writeFileSync(`./images-offline/${image.name}`, image.contents));
        }

        if (response.files) {
            mkdirp.sync('./files-offline/');
            response.files.forEach(file => fs.writeFileSync(`./files-offline/${file.name}`, file.contents));
        }
    ...
    favicons('./logo.png', {
        ...
        if (response.images) {
            ...
            response.images.forEach(image => fs.writeFileSync(`./images-online/${image.name}`, image.contents));
        }

        if (response.files) {
            mkdirp.sync('./files-online/');
            response.files.forEach(file => fs.writeFileSync(`./files-online/${file.name}`, file.contents));
        }
