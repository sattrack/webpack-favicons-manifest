        ...
        return {
            ...
            General: {
                ...
                source: (source, callback) => {
                    ...
                    if (!source || !source.length) {
                        ...
                        async.each(source, (file, cb) => readFile(file, (error, buffer) => {
                            if (error) {
                                return cb(error);
                            }
                            ...
                            cb(null);
                        }), error => callback(error || sourceset.length ? null : 'Favicons source is invalid', sourceset));
            ...
            HTML: {
                ...
                update: (document, code, tags, callback) => {
                    ...
                    async.waterfall([cb => mkdirp(path.dirname(document), cb), (made, cb) => fs.readFile(document, encoding, (error, data) => cb(null, error ? null : data)), (data, cb) => data ? updateDocument(data, code, tags, cb) : cb(null, code.join('\n')), (html, cb) => fs.writeFile(document, html, options, cb)], callback);
                }
            },
            ...
            Images: {
                create: (properties, background, callback) => {
                    ...
                    jimp = new Jimp(properties.width, properties.height, properties.transparent ? 0x00000000 : background, (error, canvas) => callback(error, canvas, jimp));
                },
                read: (file, callback) => {
                ...
                nearest: (sourceset, properties, offset, callback) => {
                    ...
                    if (svgSource) {
                        ...
                        svg2png(svgSource.file, { height, width }).then(resizedBuffer => callback(null, {
                ...
                composite: (canvas, image, properties, offset, maximum, callback) => {
                    ...
                    if (properties.mask) {
                        ...
                        async.parallel([cb => Jimp.read(circle, cb), cb => Jimp.read(overlay, cb)], (error, images) => {
